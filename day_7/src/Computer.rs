    ...
    T: ComputerIO,
{
    ...
    fn input(&mut self, parameters_modes: &Vec<ParameterMode>) -> () {
        let dest_index = self
            .get_value_at_index(self.current_intcode_pos + 1, &ParameterMode::Immediate)
            as usize;
    ...
    fn jump_if_true(&mut self, parameters_modes: &Vec<ParameterMode>) -> () {
        let is_non_zero =
            self.get_value_at_index(self.current_intcode_pos + 1, &parameters_modes[0]) != 0;

        // println!("Checking if {} is non zero: {}", self.get_value_at_index(self.current_intcode_pos + 1, &parameters_modes[0]), is_non_zero);

        if is_non_zero {
            let new_intcode_pos = self
                .get_value_at_index(self.current_intcode_pos + 2, &parameters_modes[1])
                as usize;
